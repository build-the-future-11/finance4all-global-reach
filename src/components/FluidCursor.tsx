...
>> = {
  ...
  shading: true,
  colorUpdateSpeed: 4,
  backColor: { r: 0.015, g: 0.03, b: 0.02 },
  transparent: true,
};
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      function supportRenderTextureFormat(
        ...
        return glLocal.checkFramebufferStatus(glLocal.FRAMEBUFFER) === glLocal.FRAMEBUFFER_COMPLETE;
      }
    ...
    const displayShaderSource = `
      ...
      void main () {
        vec3 c = texture2D(uTexture, vUv).rgb;
          ...
          float diffuse = clamp(dot(n, l) + 0.58, 0.58, 1.0);
          c *= diffuse;
        #endif

        // Tiny green bias, kept extremely soft.
        c.r *= 0.98;
        c.g *= 1.01;
        c.b *= 0.98;
    ...
    function generateColor(): ColorRGB {
      ...
      const h = 0.29 + Math.random() * 0.08;
      const s = 0.25 + Math.random() * 0.16;
      const v = 0.18 + Math.random() * 0.14;
      const c = hsvToRgb(h, s, v);

      return {
        r: clamp01((tint.r * 0.6 + c.r * 0.4) * 0.28),
        g: clamp01((tint.g * 0.65 + c.g * 0.35) * 0.34),
        b: clamp01((tint.b * 0.7 + c.b * 0.3) * 0.22),
      };
    }
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: ColorRGB) {
      ...
      if (splatProgram.uniforms.radius) {
        gl.uniform1f(splatProgram.uniforms.radius, radius * 1.12);
      }
      blit(dye.write);
      dye.swap();
    }
    ...
    function splatPointer(pointer: Pointer) {
      const dx = pointer.deltaX * config.SPLAT_FORCE * 0.7;
      const dy = pointer.deltaY * config.SPLAT_FORCE * 0.7;
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color);
    }

    function clickSplat(pointer: Pointer) {
      const color = generateColor();
      const dx = 4 * (Math.random() - 0.5);
      const dy = 4 * (Math.random() - 0.5);
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, color);
    }

    function ambientSplat() {
      const x = 0.5 + (Math.random() - 0.5) * 0.12;
      const y = 0.52 + (Math.random() - 0.5) * 0.1;
      const color = generateColor();
      splat(x, y, 2.0 * (Math.random() - 0.5), 2.0 * (Math.random() - 0.5), color);
    }
    ...
    function updateFrame() {
      ...
      if (ambientTimer >= 2.2) {
        ambientTimer = 0;
        if (!pointers[0].down) ambientSplat();
      }
